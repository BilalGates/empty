...
The first typed V1 payload (`password`) now has strict decoders on both platforms and dispatch from authenticated object type. A shared corpus also fixes a conservative canonical origin grammar. The TypeScript gate passes 56 core/protocol tests plus 11 extension tests; the iOS simulator passes 50 tests. The iOS preview provider compares full origins for URL service identifiers and binds direct selected identities to saved records, while domain/app identifiers fail closed. Physical-device AutoFill behavior, other payload types, signed operations, and production persistence/sync remain unverified.
