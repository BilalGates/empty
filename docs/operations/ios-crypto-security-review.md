...
- The V1 password payload now has a bounded Swift decoder and shared byte vector. `openPassword` checks the authenticated expected object type before dispatch. A shared positive/negative corpus fixes canonical wire origins, but the existing provider preview still matches hosts rather than full origins; V1 must stay disconnected until that boundary is replaced and tested. Other payload types, writer, and persistence integration remain pending.
