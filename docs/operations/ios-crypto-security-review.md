...
- An independent object-envelope vector now proves Swift can parse and write the V1 object frame, compare expected AAD, encrypt/decrypt the wrapped DEK and payload with distinct AAD suffixes, and validate deterministic CBOR. The public password writer creates a fresh DEK/nonces; no persistence integration exists. Signed operations and checkpoints remain open.
- The V1 password payload now has a bounded Swift decoder and shared byte vector. `openPassword` checks the authenticated expected object type before dispatch. A shared positive/negative corpus fixes canonical wire origins. The provider preview now matches complete origins for URL service identifiers in lists and binds direct selected identities to the saved record; domain-only/app identifiers fail closed. Direct requests do not expose the requesting URL to this extension, so OS association behavior and the full AutoFill flow still require signed physical-device evidence before V1 activation. Other payload types and persistence integration remain pending.
