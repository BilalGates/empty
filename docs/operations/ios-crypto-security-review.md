...
- The V1 password payload now has a bounded Swift decoder and shared byte vector. `openPassword` checks the authenticated expected object type before dispatch. Origin semantics, other payload types, writer, and persistence integration remain pending.
