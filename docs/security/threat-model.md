...
- El escritor Swift V1 genera DEK y nonces frescos y valida el payload `password`, pero el llamador futuro deberá imponer versiones y epochs crecientes; el codec aislado no puede detectar rollback por sí mismo.
- El codec del header de operación V1 comprueba forma y bytes deterministas, no autenticidad: una operación remota sigue siendo no confiable hasta verificar firma Ed25519, hash del artefacto, contexto AAD, secuencia/dispositivo, padres DAG y checkpoint local.
